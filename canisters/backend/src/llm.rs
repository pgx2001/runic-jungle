...
"#
    ...
    pub async fn chat(
        ...
        let system = format!(
            r#"You're an AI agent. Your name is {agent_name}. Here is some more description about you: {agent_description}. The bitcoin wallet address of user is: {user_bitcoin_address}"#
        );
        let llm_canister = Principal::from_text(LLM_CANISTER).unwrap();
