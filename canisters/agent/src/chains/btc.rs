...
use utils::{
    account_to_derivation_path, bitcoin_address_to_derivation_path, derive_public_key, ripemd160,
    sha256,
};
...
pub fn address_validation(addr: &str) -> Result<Address, String> {
    read_config(|config| {
        let bitcoin_network = match config.bitcoin_network {
            ...
            IcBitcoinNetwork::Testnet => Network::Testnet4,
...
pub fn generate_subwallet(wallet: &str) -> String {
    read_config(|config| {
        ...
        let path = bitcoin_address_to_derivation_path(wallet);
